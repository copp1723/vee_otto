...
export const fixedVehicleProcessingTask: TaskDefinition = {
  ...
  async execute(context: TaskContext): Promise<any> {
    ...
    try {
      ...
      for (let i = 0; i < maxVehicles; i++) {
        const vehicleResult = await this.processVehicle(page, vehicleLinks[i], i + 1, logger);
  ...
  async processVehicle(page: Page, vehicleLink: any, index: number, logger: any): Promise<any> {
